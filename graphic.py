...
class Graphics:
    def __init__(self, start_game_board: list[int]):
        pygame.init()
        # pygame.mixer.init()
        self.start_game_board = start_game_board.copy()
        ...
        while running:
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    is_mouse_pressed = False
                    if not ((selected_figure.rank == 1 and is_attackers_move) or (
                            selected_figure.rank == -1 and not is_attackers_move) or (
                                    selected_figure.rank == -2 and not is_attackers_move)):
                        break
                    ...
                    if position[0] == selected_figure.rect.x and position[1] == selected_figure.rect.y:
                        ...
                        selected_figure = None
                        is_attackers_move = not is_attackers_move
                        if result == 1:
    ...
    def __end_message(self, message, screen):
        self.board = self.start_game_board.copy()
        self.all_sprites = self.init_sprites()
        running = True
        while running:
            for e in pygame.event.get():
